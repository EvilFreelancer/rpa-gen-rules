...
1. `.claude-plugin/marketplace.json` (catalog) - in the `rpa-gen-rules` entry set `version` to the new number and
...
```bash
grep -A3 '"name": "rpa-gen-rules"' .claude-plugin/marketplace.json
